...
export default function DashboardPage() {
  const [stats, setStats] = useState({ aktif: 0, terminasi: 0, totalWk: 0, totalDok: 0 });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function load() {
      try {
        const [total, aktif, terminasi, dokumen] = await Promise.all([
          ...
          supabase.from("rm_information_item").select("*", { count: "exact", head: true }),
        ]);
        setStats({
          totalWk: total.count ?? 0,
          aktif: aktif.count ?? 0,
          terminasi: terminasi.count ?? 0,
          totalDok: dokumen.count ?? 0,
        });
  ...
  const cards = [
    ...
    { label: "Total Dokumen", value: stats.totalDok, color: "#42a5f5", sub: "WPB + AFE termigrasikan" },
  ];
      ...
      <div style={{ padding:20, display:"flex", flexDirection:"column", gap:16 }}>
        ...
        <div>
          ...
          <div style={{ background:"#2c2c2c", border:"2px dashed #3a3a3a", borderRadius:6, padding:36, textAlign:"center", cursor:"pointer" }}>
            <div style={{ fontSize:13, color:"#757575" }}>Seret &amp; lepas file PDF di sini, atau <span style={{ color:"#66bb6a", fontWeight:500 }}>pilih file</span></div>
