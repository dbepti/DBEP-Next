...
export default function CadanganPage() {
  ...
  const [activeTab, setActiveTab] = useState<"dokumen"|"volumetrik">("dokumen");
  const [cadangan, setCadangan] = useState<any[]>([]);
  ...
  useEffect(() => {
    sb.from("v_dokumen").select("wkid,nama_wk").eq("item_type","CADANGAN").not("wkid","is",null)
      .then(({ data }) => {
        if (!data) return;
  ...
  // Load data cadangan volumetrik saat dokumen dipilih
  useEffect(()=>{
    if (!selected?.wkid) { setCadangan([]); return; }
    setLoadingCad(true);
    sb.from("v_cadangan").select("*").eq("wkid",selected.wkid)
      .order("lapangan",{ascending:true})
      .then(({data})=>{ setCadangan(data??[]); setLoadingCad(false); });
  }, [selected]);
  ...
  const fmt = (v:any) => v ? parseFloat(v).toFixed(1) : "—";

  // Pisah minyak & gas
  const cadOil = cadangan.filter(c=>c.product_type==="OIL");
  const cadGas = cadangan.filter(c=>c.product_type==="GAS");
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",background:"#1a1a1a",overflow:"hidden"}}>
      <div style={{padding:"8px 16px",borderBottom:"1px solid #2a2a2a",background:"#212121",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",flexShrink:0}}>
        <span style={{fontSize:13,color:"#e0e0e0",fontWeight:500}}>Cadangan Migas</span>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{flex:"0 0 55%",display:"flex",flexDirection:"column",overflow:"hidden",borderBottom:"1px solid #2a2a2a"}}>
          ...
          <div style={{padding:"5px 16px",borderTop:"1px solid #222",background:"#1e1e1e",display:"flex",alignItems:"center",flexShrink:0}}>
            <span style={{color:"#3a3a3a",fontSize:11}}>Hal {page+1}/{totalPages||1} · {total} dokumen</span>
        ...
        <div style={{flex:"0 0 45%",display:"flex",flexDirection:"column",overflow:"hidden",background:"#1c1c1c"}}>
          <div style={{padding:"0 16px",background:"#1e1e1e",borderBottom:"1px solid #252525",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
            <div style={{display:"flex"}}>
              {(["dokumen","volumetrik"] as const).map(tab=>(
                <button key={tab} onClick={()=>setActiveTab(tab)}
                  ...
                  {tab==="dokumen"?"Dokumen":"Cadangan Volumetrik"+(cadangan.length?" ("+cadangan.length/2+" lapangan)":"")}
                </button>
              ))}
            </div>
          ...
          <div style={{flex:1,overflow:"auto",padding:"16px 20px"}}>
            {!selected?(
              ...
              <>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"2px 32px",marginBottom:12}}>
                  {[
                    ...
                    <div key={l as string} style={{display:"flex",borderBottom:"1px solid #222",padding:"6px 0"}}>
                      <span style={{fontSize:12,color:"#555",width:120,flexShrink:0}}>{l}</span>
                ...
                {cadangan.length>0&&(
                  <div style={{padding:"8px 12px",background:"#1a1a1a",borderRadius:6,border:"1px solid #252525"}}>
                    <span style={{fontSize:11,color:"#444"}}>Tersedia data cadangan untuk </span>
                    <button onClick={()=>setActiveTab("volumetrik")} style={{background:"none",border:"none",color:"#42a5f5",fontSize:11,cursor:"pointer",padding:0}}>{cadangan.length/2} lapangan</button>
              ...
              <>
                <div style={{fontSize:13,color:"#66bb6a",fontWeight:500,marginBottom:16}}>
                  {selected.nama_wk??selected.wkid}
                  <span style={{fontSize:11,color:"#444",marginLeft:8,fontWeight:400}}>({cadangan.length/2} lapangan)</span>
                </div>
                {loadingCad?<div style={{color:"#333",fontSize:12}}>Memuat...</div>:(
                  <>
                    {/* Minyak */}
                    {cadOil.length>0&&(
                      <div style={{marginBottom:16}}>
                        <div style={{fontSize:11,color:"#ffa726",marginBottom:6,fontWeight:500}}>🛢 Minyak (MMBO)</div>
                        <div style={{display:"grid",gridTemplateColumns:"1fr 70px 70px 70px 70px",gap:4,fontSize:10,color:"#3a3a3a",padding:"3px 8px",textTransform:"uppercase"}}>
                          <span>Lapangan</span><span style={{textAlign:"right"}}>1P</span><span style={{textAlign:"right"}}>2P</span><span style={{textAlign:"right"}}>3P</span><span style={{textAlign:"right"}}>Pot.</span>
                        </div>
                        {cadOil.map(c=>(
                          <div key={c.resent_id} style={{display:"grid",gridTemplateColumns:"1fr 70px 70px 70px 70px",gap:4,padding:"4px 8px",borderRadius:3,background:"#1a1a1a",marginBottom:1,fontSize:11}}>
                            <span style={{color:"#bbb",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.nama_lapangan??c.lapangan}</span>
                            <span style={{color:"#ffa726",textAlign:"right"}}>{fmt(c.a1p)}</span>
                            <span style={{color:"#ff8f00",textAlign:"right"}}>{fmt(c.a2p)}</span>
                            <span style={{color:"#e65100",textAlign:"right"}}>{fmt(c.a3p)}</span>
                            <span style={{color:"#555",textAlign:"right"}}>{fmt(c.potensial)}</span>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    {cadGas.length>0&&(
                      <div>
                        <div style={{fontSize:11,color:"#42a5f5",marginBottom:6,fontWeight:500}}>⛽ Gas (BSCF)</div>
                        <div style={{display:"grid",gridTemplateColumns:"1fr 70px 70px 70px 70px",gap:4,fontSize:10,color:"#3a3a3a",padding:"3px 8px",textTransform:"uppercase"}}>
                          <span>Lapangan</span><span style={{textAlign:"right"}}>1P</span><span style={{textAlign:"right"}}>2P</span><span style={{textAlign:"right"}}>3P</span><span style={{textAlign:"right"}}>Pot.</span>
                        </div>
                        {cadGas.map(c=>(
                          <div key={c.resent_id+"g"} style={{display:"grid",gridTemplateColumns:"1fr 70px 70px 70px 70px",gap:4,padding:"4px 8px",borderRadius:3,background:"#1a1a1a",marginBottom:1,fontSize:11}}>
                            <span style={{color:"#bbb",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.nama_lapangan??c.lapangan}</span>
                            <span style={{color:"#42a5f5",textAlign:"right"}}>{fmt(c.a1p)}</span>
                            <span style={{color:"#1976d2",textAlign:"right"}}>{fmt(c.a2p)}</span>
                            <span style={{color:"#0d47a1",textAlign:"right"}}>{fmt(c.a3p)}</span>
                            <span style={{color:"#555",textAlign:"right"}}>{fmt(c.potensial)}</span>
                          </div>
                        ))}
                      </div>
