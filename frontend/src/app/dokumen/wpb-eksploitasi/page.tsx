...
export default function Page() {
  ...
  useEffect(() => {
    sb.from("v_dokumen").select("wkid,nama_wk").eq("item_type","WPB_EKSPLOITASI").not("wkid","is",null)
      .then(({ data }) => {
        if (!data) return;
  ...
  const load = useCallback(async () => {
    setLoading(true);
    let q = sb.from("v_dokumen").select("*",{count:"exact"}).eq("item_type","WPB_EKSPLOITASI")
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",background:"#1a1a1a",overflow:"hidden"}}>
      <div style={{padding:"8px 16px",borderBottom:"1px solid #2a2a2a",background:"#212121",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",flexShrink:0}}>
        <span style={{fontSize:13,color:"#e0e0e0",fontWeight:500}}>WP&B Eksploitasi</span>
