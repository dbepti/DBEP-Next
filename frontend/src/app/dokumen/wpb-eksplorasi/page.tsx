...
export default function Page() {
  ...
  useEffect(() => {
    sb.from("v_dokumen").select("wkid,nama_wk").eq("item_type","WPB_EKSPLORASI").not("wkid","is",null)
      .then(({ data }) => {
        if (!data) return;
  ...
  const load = useCallback(async () => {
    setLoading(true);
    let q = sb.from("v_dokumen").select("*",{count:"exact"}).eq("item_type","WPB_EKSPLORASI")
