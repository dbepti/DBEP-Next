...
export default function WKBaruPage() {
  ...
  useEffect(() => {
    sb.from("LAND_RIGHT").select("LAND_RIGHT_ID")
      ...
      .then(({ data }) => {
        if (data?.[0]) {
          const last = data[0].LAND_RIGHT_ID as string;
          const num  = parseInt(last.replace(/[^0-9]/g,"")) || 0;
          setWkid(`${last.replace(/[0-9]+$/,"")}${String(num+1).padStart(4,"0")}`);
        }
      });
  }, []);
  ...
  async function handleSubmit() {
    ...
    if (!kkksSelected) { setMsg("❌ Operator KKKS wajib dipilih dari daftar"); return; }
    setSaving(true); setMsg("");
    try {
      // Re-validasi WKID tidak konflik dengan data terbaru
      const { data: chk } = await sb.from("LAND_RIGHT").select("LAND_RIGHT_ID")
        .eq("LAND_RIGHT_ID", wkid.trim().toUpperCase()).limit(1);
      if (chk && chk.length > 0) {
        setMsg("❌ WKID sudah dipakai, refresh halaman untuk dapat WKID baru");
        setSaving(false); return;
      }
      const res = await fetch(`${API}/wk/`, {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({
          wkid: wkid.trim().toUpperCase(), nama_wk: namaWk.trim(),
          tipe_kontrak: tipeKontrak, lokasi, fase,
          tgl_ttd: tglTtd||null, tgl_efektif: tglEfektif||null, tgl_berakhir: tglBerakhir||null,
      ...
      setMsg("✅ " + data.message);
      setSaved(true);
      setTimeout(() => router.push(`/wk/${wkid.trim().toUpperCase()}`), 800);
        ...
        <div style={{ maxWidth:600 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"160px 1fr", gap:12 }}>
            <Field label="WKID" required hint="auto-generate">
              <input value={wkid} onChange={e=>setWkid(e.target.value.toUpperCase())} style={{ ...inp, fontFamily:"monospace", color:"#66bb6a" }} />
            </Field>
            <Field label="Nama Wilayah Kerja" required>
              <input value={namaWk} onChange={e=>setNamaWk(e.target.value)} placeholder="contoh: WEST BEDUGA" style={inp} />
            </Field>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12 }}>
            <Field label="Tipe Kontrak" required>
              <select value={tipeKontrak} onChange={e=>setTipeKontrak(e.target.value)} style={{ ...inp }}>
                {TIPE_LIST.map(t=><option key={t} value={t}>{t}</option>)}
              </select>
            </Field>
            <Field label="Lokasi">
              <select value={lokasi} onChange={e=>setLokasi(e.target.value)} style={{ ...inp }}>
          ...
          <div style={{ fontSize:11, fontWeight:600, color:"#555", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:14, marginTop:4 }}>Tanggal Kontrak</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12 }}>
            <Field label="Tgl Ditandatangani"><input type="date" value={tglTtd} onChange={e=>setTglTtd(e.target.value)} style={inp} /></Field>
            <Field label="Tgl Efektif"><input type="date" value={tglEfektif} onChange={e=>setTglEfektif(e.target.value)} style={inp} /></Field>
            <Field label="Tgl Berakhir"><input type="date" value={tglBerakhir} onChange={e=>setTglBerakhir(e.target.value)} style={inp} /></Field>
          </div>
          <Field label="Luas Awal (km²)">
            <input type="number" value={luasKm2} onChange={e=>setLuasKm2(e.target.value)} placeholder="contoh: 6225" style={{ ...inp, maxWidth:200 }} />
          </Field>
          <div style={{ fontSize:11, fontWeight:600, color:"#555", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:14, marginTop:4 }}>KKKS Operator</div>
          <Field label="Cari & Pilih KKKS Operator" required>
            <input value={kkksSearch} onChange={e=>{ setKkksSearch(e.target.value); setKkksSelected(null); }}
              placeholder="Ketik nama KKKS... (min 2 huruf)" style={inp} />
            {kkksOptions.length > 0 && !kkksSelected && (
              <div style={{ background:"#1a1a1a", border:"1px solid #383838", borderRadius:4, marginTop:2, maxHeight:200, overflowY:"auto", zIndex:10, position:"relative" }}>
            ...
            {kkksSelected && (
              <div style={{ marginTop:6, padding:"7px 10px", background:"#1b5e2020", border:"1px solid #2e7d3244", borderRadius:4, fontSize:11, color:"#81c784", display:"flex", alignItems:"center" }}>
                <span style={{ flex:1 }}>✓ {kkksSelected.nama} <span style={{ color:"#555", fontSize:10 }}>{kkksSelected.id}</span></span>
                <button onClick={()=>{ setKkksSelected(null); setKkksSearch(""); }} style={{ background:"transparent", border:"none", color:"#555", cursor:"pointer", fontSize:13 }}>✕</button>
              </div>
            )}
          </Field>
          {msg && (
          ...
          <div style={{ display:"flex", gap:10, paddingTop:8, borderTop:"1px solid #2a2a2a" }}>
            <button onClick={handleSubmit} disabled={saving || saved} style={{ padding:"8px 28px", borderRadius:4, background:"#1b5e20", border:"1px solid #2e7d32", color:"#a5d6a7", cursor:"pointer", fontSize:13, fontWeight:500, opacity:saving?0.6:1 }}>
              {saving?"Menyimpan...":"Simpan WK Baru"}
            </button>
            <Link href="/wk" style={{ padding:"8px 16px", borderRadius:4, background:"transparent", border:"1px solid #383838", color:"#757575", fontSize:13, textDecoration:"none" }}>Batal</Link>
          </div>
        </div>
      </div>
    </div>
