...
export default function PetaWK() {
  ...
  const loadWK = useCallback(async (map: any) => {
    ...
    map.addSource("wk", { type: "geojson", data: { type: "FeatureCollection", features } as any });
    map.addLayer({ id: "wk-fill", type: "fill", source: "wk", paint: { "fill-color": ["case", ["==", ["get", "active_ind"], "Y"], "#1b5e20", "#424242"], "fill-opacity": 0.4 } });
    map.addLayer({ id: "wk-line", type: "line", source: "wk", paint: { "line-color": ["case", ["==", ["get", "active_ind"], "Y"], "#66bb6a", "#616161"], "line-width": 1 } });
  ...
  const loadCekungan = useCallback(async (map: any) => {
    ...
    const { data } = await sb.from("v_cekungan").select("cekungan_id,nama,luas_km2,klasifikasi_kode,geom,has_geom");
    if (!data) return;
    setCekunganList(data);

    const features = data.filter((c: any) => c.geom).map((c: any) => ({
      type: "Feature",
    ...
    if (!map.getSource("cekungan")) {
      map.addSource("cekungan", { type: "geojson", data: { type: "FeatureCollection", features } as any });
  ...
  useEffect(() => {
    ...
    if (showCekungan) {
      loadCekungan(map);
      if (map.getLayer("cekungan-fill")) {
        map.setLayoutProperty("cekungan-fill", "visibility", "visible");
        map.setLayoutProperty("cekungan-line", "visibility", "visible");
      }
    } else {
      if (map.getLayer("cekungan-fill")) {
        map.setLayoutProperty("cekungan-fill", "visibility", "none");
        map.setLayoutProperty("cekungan-line", "visibility", "none");
      }
    }
  }, [showCekungan]);
  ...
  useEffect(() => {
    ...
    if (mode === "digitasi") {
      ...
      const handleClick = (e: any) => {
        ...
        el.style.cssText = "width:8px;height:8px;background:#ff8a65;border:1px solid #fff;border-radius:50%;";
        const marker = new (map.constructor as any)._maplibregl?.Marker
          ? null
          : null;
